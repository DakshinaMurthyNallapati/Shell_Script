...
LOG_FILE=$(echo $0 | awk -F "/"'{print $n}'cut -d "." -f1)
...
mkdir -p /home/ec2-user/shellscript-logs
    ...
    if [ ! -d $DEST_DIR ] && [ ! -d $SOURCE_DIR ]
        then
            echo -e "$SOURCE_DIR $DEST_DIR $R Both directories does not exist..$N Please check"
            exit 1
...
if [ -n "$FILES" ]
    then 
        ...
        find $SOURCE_DIR -name "*.log" -mtime +$DAYS | zip -@ "$ZIP_FILE" 
            if [ -f "$ZIP_FILE" ]
                then 
                    echo -e "Successfully created the zip file for files older than $DAYS"
